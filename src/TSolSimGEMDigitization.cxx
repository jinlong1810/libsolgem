  ...
  for (UInt_t ih = 0; ih < nh; ++ih) {
	...
	if( !time_set[itime] ) {
	  ...
	  if( is_background ) {
		...
		//event_time[itime] = fTrnd.Uniform(fGateWidth + 5.*fEleSamplingPeriod)
	...
	for (Int_t j = 0; j < nstrips; j++){
	  ...
	  for (Int_t b = 0; b < fEleSamplingPoints; b++){
		...
		if (fChipMode == 0){
		  ...
		  //test for VMM3
		  pulse = TSolSimAux::VMMPulseShape (fEleSamplingPeriod * b - t0, us );        
		}
		//cout<<"b="<<b<<", t0 = "<<t0<<", pulse="<<pulse<<endl;
